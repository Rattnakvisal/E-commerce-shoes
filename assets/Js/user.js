...
const ModalController = {
    ...
    reset() {
        ...
        Elements.submitBtn.className = 'px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition';
    ...
    setupForEdit(user) {
        ...
        Elements.submitBtn.className = 'px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition';
        Elements.passwordLabel.textContent = 'Password (leave blank to keep current)';
...
const UserManager = {
    ...
    async changeRole(id, userName) {
        ...
        const data = await ApiService.request({ action: 'update_role', user_id: id, role });
        Swal.close();

        if (data?.success) {
            await SwalHelpers.success('Role Updated!', `User role changed to ${role}.`);
    ...
    updateRoleInUI(id, role) {
        const roleSpan = document.querySelector(`tr[data-id="${id}"] .role-cell span`);
        if (!roleSpan) return;
        ...
        roleSpan.className = `px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${roleClasses[role] || 'role-customer'}`;
    }
};
...
const FormValidator = {
    validate() {
        ...
        if (Elements.formAction.value === 'create' &&
...
const FormHandler = {
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            if (data?.success) {
                const message = Elements.formAction.value === 'create'
                    ? 'User Created!'
                    : 'User Updated!';
...
function setupEventListeners() {
    ...
    // Form events
    Elements.userForm?.addEventListener('submit', (e) => FormHandler.handleSubmit(e));
